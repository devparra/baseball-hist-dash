...
def update_figure5(selected_team, year_range):
    ...
    fig5.update_layout(hovermode="x",title="K/BB Ratio with ERA Bubble",
        font={'color':'darkslategray'},paper_bgcolor='white',plot_bgcolor='#f8f5f0')
...
def update_figure8(player, selected_team, year_range):
    ...
    fig8 = go.Figure(data=[
            go.Scatter(name='OPS', x=filter_batter.year, y=OPS, mode='lines+markers', marker_color='green',
                hovertemplate = 'OPS: %{text:.3f}<extra></extra><br>',text = ['{}'.format(i) for i in OPS]),
            go.Scatter(name='SLG', x=filter_batter.year, y=SLG, mode='lines+markers', marker_color='dodgerblue',
                hovertemplate = 'SLG: %{text:.3f}<extra></extra><br>',text = ['{}'.format(i) for i in SLG]),
...
def update_field_table(player, selected_team, year_range):
    ...
    # drop unneccesary columns
    data_filter = filter_player.drop(columns=['player_id', 'team_id', 'stint', 'league_id', 'inn_outs', 'gs', 'pb', 'wp', 'sb', 'cs', 'zr'])
...
def update_pitch_table(player, selected_team, year_range):
    ...
    # drop unneccesary columns
    data_filter = filter_batter.drop(columns=['player_id', 'team_id', 'stint', 'league_id', 'ipouts', 'baopp'])

    # Set empty list
    data_note = []
    # if data filter is empty, append and return notice
    if data_filter.empty:
        data_note.append(html.Div(dbc.Alert('No pitching data is available for selected player.', color='warning'),))
        return data_note
...
def update_pos_dropdown(player, selected_team, year_range):
    ...
    positions = [{'label': k, 'value': v }for k, v in zip(filter_player.pos, filter_player.pos)]
    # set empty list
    pos_list = []
